...
// Fungsi Deep Merge konfigurasi agar tidak ada field/key default yang hilang
...
function loadSavedOrUrlConfig() {
  ...
  try {
    ...
    if (savedConfig) {
      ...
      // Bersihkan jika tersimpan link pixabay lama atau format lama
      if (parsed.music && (parsed.music.audioUrl?.includes("pixabay.com") || parsed.music.audioUrl === "builtin" || !parsed.music.audioUrl)) {
        parsed.music.audioUrl = "music.mp3";
        parsed.music.title = "Aesthetic Lofi Beats";
        parsed.music.artist = "Chilled Vibes ✨";
        localStorage.setItem("custom_site_config", JSON.stringify(parsed));
      }
      currentConfig = deepMerge(CONFIG, parsed);
    }
  } catch (e) {
  ...
  try {
    ...
    if (sharedData) {
      ...
      if (decoded.music && (decoded.music.audioUrl?.includes("pixabay.com") || decoded.music.audioUrl === "builtin")) {
        decoded.music.audioUrl = "music.mp3";
      }
      currentConfig = deepMerge(currentConfig, decoded);
    }
  } catch (e) {
...
function initAudio() {
  ...
  let rawUrl = (music.audioUrl || "music.mp3").trim();
  if (!rawUrl || rawUrl.includes("pixabay.com") || rawUrl === "builtin") {
    rawUrl = "music.mp3";
  }

  isUsingSynth = false;
  const vol = music.defaultVolume ?? 0.6;
  ...
  if (audioPlayer) {
    ...
    audioPlayer.onerror = () => {
      console.warn("Audio gagal dimuat, mengaktifkan synth engine sebagai backup:", rawUrl);
      isUsingSynth = true;
      synthEngine.play();
    };
  }
  ...
  if (titleEl) titleEl.textContent = music.title || "Aesthetic Lofi Beats";
  if (artistEl) artistEl.textContent = music.artist || "Chilled Vibes ✨";
...
function playAudio() {
    ...
    if (isUsingSynth) {
      ...
      if (playPromise !== undefined) {
        playPromise
          ...
          .catch(err => {
            console.log("Autoplay ditahan browser, menunggu interaksi pertama:", err);
  ...
  const attachFirstInteraction = () => {
    ...
    const cleanup = () => {
      ["pointerdown", "click", "touchstart", "touchend", "keydown", "scroll"].forEach(evt => {
        window.removeEventListener(evt, handleGesture, { capture: true });
      });
    };

    ["pointerdown", "click", "touchstart", "touchend", "keydown", "scroll"].forEach(evt => {
      window.addEventListener(evt, handleGesture, { capture: true, once: true });
    });
  };
